...
--  Messages
--====================================================================================
function getMessages(phone, cbr)
    local task = Task(cbr)
                ...
                if is_valid == true then
                    ...
                    if srcTo ~= nil then
                        ...
                        if checkIfInCall(srcTo) then
                            TriggerClientEvent("pNotify:SendNotification", source, {text = "<h3>📱 Telefonsvarer</h3> <br> <p>Nummeret er optaget</P>", type = "error", timeout = 9000, layout = "bottomright"})
                        else
...
AddEventHandler('gcPhone:allUpdate', function(user_id, source)
    ...
    vRP.getUserIdentity({user_id, function(identity)
        if identity then
            ...
            getMessages(phone, function(messages)
                if messages ~= nil then
                    TriggerClientEvent("gcPhone:allMessage", lsource, messages)
                end
            end)
