    ...
    if item ~= nil and item.price >= 0 then
        ...
        if vRP.tryFullPayment({nuser_id,tonumber(item.price)}) then
            ...
            if command ~= "" then
                ...
                TriggerClientEvent("pNotify:SendNotification", source,{text = "Køretøjet har fået "..item.title.."<br>"..((lvl ~= "off") and "Betalt: <b style='color: #DB4646'>"..item.price.."</b> DKK" or ""), type = "success", queue = "global", timeout = 5000, layout = "centerRight",animation = {open = "gta_effects_open", close = "gta_effects_close"}})
            end
        else
