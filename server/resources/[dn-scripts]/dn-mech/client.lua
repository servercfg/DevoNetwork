...
RegisterCommand("mech", function()
	if not vRP.isInComa() and not vRP.isHandcuffed() then
		...
		if job == "Mekaniker" or job == "Mekaniker Chef" then
			...
			if veh ~= 0 then
				...
				if vehname ~= nil then
					...
					if luser_id then
						...
						TriggerEvent("pNotify:SendNotification",{text = "Kunne ikke finde ejeren af køretøjet!",type = "error",timeout = (5000),layout = "centerRight",queue = "global",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
					end
				else
					TriggerEvent("pNotify:SendNotification",{text = "Kunne ikke finde ejeren af køretøjet!",type = "error",timeout = (5000),layout = "centerRight",queue = "global",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
				end
			else
				TriggerEvent("pNotify:SendNotification",{text = "Du skal sidde i en bil",type = "error",timeout = (5000),layout = "centerRight",queue = "global",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
			end
		end
	end
	CancelEvent()
...
RegisterNUICallback( "ButtonClick", function( data, cb )
		...
		if DoesEntityExist(veh) then
			if vehname ~= "" then
				if IsToggleModOn(veh,18) then
					...
					TriggerEvent("pNotify:SendNotification",{text = "Bilen er allerede uden turbo.",type = "info",timeout = (3000),layout = "centerRight",queue = "global",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"},killer = true})
				end
			end
		end
	elseif ( data == "turbo2" ) then
		...
		if DoesEntityExist(veh) then
			if vehname ~= "" then
				if IsToggleModOn(veh,18) then
					TriggerEvent("pNotify:SendNotification",{text = "Bilen er allerede med turbo.",type = "info",timeout = (3000),layout = "centerRight",queue = "global",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"},killer = true})
				else
	...
	if DoesEntityExist(veh) then
		if vehname ~= "" then
			if GetVehicleMod(veh, mod) == lvl then
				TriggerEvent("pNotify:SendNotification",{text = "Bilen har allerede det udstyr",type = "info",timeout = (3000),layout = "centerRight",queue = "global",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"},killer = true})
			else
				if toggle then
...
RegisterCommand("impound", function()
	if not vRP.isInComa() and not vRP.isHandcuffed() then
		...
		if job == "Mekaniker" or job == "Mekaniker Chef" or checkJob then
			...
			if veh ~= 0 then
				...
				if vehname ~= nil then
					...
					if luser_id then
						if checkJob and vRPex.getAmountOnline({"mechanic"}) > 1 then
							TriggerEvent("pNotify:SendNotification",{text = "Ring til en mekaniker!",type = "error",timeout = (5000),layout = "centerRight",queue = "global",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
							return;
						end
						TriggerServerEvent("cn-mech:impoundveh",luser_id,plate,vehname,1,VehToNet(veh))
					else
						vRP.despawnNetVehicle({VehToNet(veh)})
						TriggerEvent("pNotify:SendNotification",{text = "Kunne ikke finde ejeren af køretøjet</br>Køretøj fjernet.",type = "error",timeout = (5000),layout = "centerRight",queue = "global",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
					end
				else
					vRP.despawnNetVehicle({VehToNet(veh)})
					TriggerEvent("pNotify:SendNotification",{text = "Kunne ikke finde ejeren af køretøjet</br>Køretøj fjernet.",type = "error",timeout = (5000),layout = "centerRight",queue = "global",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
				...
				TriggerEvent("pNotify:SendNotification",{text = "Kunne ikke finde køretøj",type = "error",timeout = (5000),layout = "centerRight",queue = "global",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
			end
		end
	end
	CancelEvent()
