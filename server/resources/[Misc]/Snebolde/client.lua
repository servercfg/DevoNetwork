...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if IsNextWeatherType('XMAS') then -- check for xmas weather type
            ...
            if IsControlJustReleased(0, 311) and not IsPedInAnyVehicle(GetPlayerPed(-1), true) and not IsPlayerFreeAiming(PlayerId()) and not IsPedSwimming(PlayerPedId()) and not IsPedSwimmingUnderWater(PlayerPedId()) and not IsPedRagdoll(PlayerPedId()) and not IsPedFalling(PlayerPedId()) and not IsPedRunning(PlayerPedId()) and not IsPedSprinting(PlayerPedId()) and GetInteriorFromEntity(PlayerPedId()) == 0 and not IsPedShooting(PlayerPedId()) and not IsPedUsingAnyScenario(PlayerPedId()) and not IsPedInCover(PlayerPedId(), 0) then -- check if the snowball should be picked up
...
function showHelpNotification()
    ...
    AddTextComponentSubstringPlayerName("Tryk ~INPUT_REPLAY_SHOWHOTKEY~ for at samle en snebold op!")
